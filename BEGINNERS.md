...
作成後は検証、GitHubへのpush、GitHub Pagesのデプロイ確認まで行ってください。
```
...
公開記事として追加し、GitHub Pagesで読める状態まで進めてください。
```

このリポジトリでは、記事作成を明示的に依頼すると、Codexは原則として次の作業まで自動で行います。

1. 記事の調査と作成
2. 書式、型、リンク、ビルドの検証
3. Gitへのコミット
4. GitHubへのpush
5. GitHub Pagesのデプロイ成功確認
6. 公開URLの案内

通常、ユーザーが`git add`、`git commit`、`git push`を操作する必要はありません。公開後はCodexが案内した記事URLを再読み込みしてください。

下書きだけを作りたい場合は、依頼時に「`draft: true`で作成し、pushしないでください」と明記します。
